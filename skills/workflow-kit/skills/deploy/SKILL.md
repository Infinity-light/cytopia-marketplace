...
在执行 deploy 前，按条件运行 `db-migration-safety` 预检脚本。

### 触发前提（必须同时满足）

1. **当前动作是 deploy 前置检查（deploy intent）**
2. **检测到数据库变更风险信号**（如迁移文件 / DDL / schema 变更）
...
| 迁移护栏返回 BLOCK | 存在高风险 DB 变更且回滚/备份证据不足 | 立即暂停 deploy，补齐 rollback 与 backup/runbook 后重跑 preflight |
